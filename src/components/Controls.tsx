...
export default function Controls() {
    ...
    const { train, isTraining, setIsTraining } = useNeuralNet();
    useEffect(() => {
    ...
    return (
        <div className="flex flex-col gap-2">
            <button className="btn btn-neutral" onClick={() => setIsTraining(!isTraining)}>
                {isTraining ? <PauseIcon className="w-5 h-5 fill-current"/> : <PlayIcon className="w-6 h-6 fill-current"/>}
            </button>
